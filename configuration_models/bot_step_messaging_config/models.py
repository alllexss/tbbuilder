...
class StepMessagingConfig(models.Model):
    ...
    send_to_admins = models.BooleanField(verbose_name="Send user replies to admin accounts?", default=True)
    ...
    def save(self, *args, **kwargs):
        self.start_message = self.start_message.replace("/", "") if self.start_message[0] == "/" else self.start_message
        super().save(*args, **kwargs)
        bot_settings_model.generate_bot_config()


class StepMessagingStep(models.Model):
    step_messaging_config = models.ForeignKey(StepMessagingConfig, on_delete=models.CASCADE)
    step_name = models.CharField(verbose_name="Step name", unique=True, max_length=120, blank=False)
...
class StepMessagingMedia(models.Model):
    step = models.ForeignKey(StepMessagingStep, on_delete=models.CASCADE)
    media_file = models.FileField(verbose_name="Add your media file to Bot message", upload_to="bot-media-files")
